...
bool check_gps_data(gps_data data) {
	if (data.altitude == 0 || data.height == 0 ||
...
string get_message() {
	...
	if (gps_data::count == 0) {
		...
		message = "$GPGGA,184911.211,4523.444,N,06055.151,W,1,12,1.0,0.0,M,0.0,M,,*7D";
	}
...
bool send_message(gps_data decoded_data) {
	...
	container.AddKeyValuePair(data_types.altitude, decoded_data.altitude); //0?
